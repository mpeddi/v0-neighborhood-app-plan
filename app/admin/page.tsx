...
export default async function AdminPage() {
  ...
  // Get statistics
  const { count: totalResidences } = await supabase.from("residences").select("*", { count: "exact", head: true })
  ...
  const { count: totalEvents } = await supabase.from("calendar_events").select("*", { count: "exact", head: true })
  ...
  const { count: totalHelpRequests } = await supabase.from("help_requests").select("*", { count: "exact", head: true })
  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-blue-50 to-green-50">
      ...
      <main className="container mx-auto px-4 py-8 max-w-7xl">
        ...
        <AdminDashboard stats={stats} recentEvents={recentEvents || []} residences={allResidences || []} />
      </main>
    </div>
  )
}
